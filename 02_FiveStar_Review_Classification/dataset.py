...
class TextLoader:
  ...
  def punc_remove(self,text,correct=False):
    ...
    for i in range(len(text)):
      ...
      if len(seq_)<1:seq_.append("unknown")
      text_seg.extend(seq_)
      text_seg_idx.append(text_seg_idx[-1]+len(seq_))

    text_corr= fastpunct.punct(text_seg, correct=True) if correct else text_seg
      ...
      for token in sentence.lower().split(" "):
        try:
          ...
          if build:
            ...
            self.vectors.append(np.random.normal(scale=0.6, size=(self.embed_size, )))
          elif not build:
            tokens.append("<unk>")
  ...
  def build_vocab(self,text,embed_size,vocab_file=None,glove_file=None,write_file=None,correct=False):
    ...
    if vocab_file is not None:
      ...
      self.vectors.append(np.random.normal(scale=0.6, size=(embed_size, )))  
